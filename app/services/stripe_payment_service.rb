  ...
  def self.process_payment_paid(payment)
    ...
    when 'Package'
      ...
      if user
        ...
        Rails.logger.info "User #{user.id} credits increased by #{package.articles_count} (Package: #{package.name})"
        
        # TODO: 可以在这里发送通知邮件
        # UserMailer.package_purchased(user, package).deliver_later
      end
    end
  end
end
